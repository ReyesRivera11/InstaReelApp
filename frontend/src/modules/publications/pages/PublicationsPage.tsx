...
export function PublicationsPage() {
  ...
  const getClientName = (clientId: number) => {
    ...
    return client ? `${client.name} (@${client.username})` : "Cliente desconocido"
  }

  const getStatusBadge = (status: string) => {
    switch (status) {
      case "scheduled":
        return <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">Programado</span>
      case "published":
        return <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs">Publicado</span>
      case "failed":
        return <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs">Fallido</span>
      default:
  ...
  const filteredPublications = publications.filter((pub) => {
    const matchesSearch =
      ...
      getClientName(pub.clientId).toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const renderCalendarView = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          <div className={`text-sm mb-2 ${isToday ? "text-purple-600" : "text-muted-foreground"}`}>{day}</div>
            ...
            {pubs.slice(0, 3).map((pub) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-muted-foreground line-clamp-1 mt-0.5">{client?.name}</p>
    ...
    <>
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Historial de Publicaciones</h1>
            <p className="text-muted-foreground mt-1">Visualiza y administra todas tus publicaciones programadas</p>
          </div>
            ...
            {viewMode === "table" && (
              <div className="bg-card rounded-lg border border-border">
                <div className="p-6 border-b border-border">
                  <h2 className="text-xl font-semibold">{sortedPublications.length} Publicaciones</h2>
                  <p className="text-sm text-muted-foreground">Listado completo de todas tus publicaciones</p>
                </div>
                <div className="p-6">
                        ...
                        <tbody>
                          {sortedPublications.map((pub) => (
                            <tr key={pub.id} className="border-b border-border hover:bg-accent/50">
                              ...
                              <td className="p-4">
                                <p className="text-sm">{getClientName(pub.clientId)}</p>
                              </td>
                              <td className="p-4">
                                <div>
                                  <p className="text-sm">
                ...
                <div className="p-6 border-b border-border">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Vista de calendario de publicaciones programadas</p>
                    </div>
